#Code for PID Longitudinal control and MPC lateral control UofT course-1
...
path.append(r"C:\Users\Lakshmikanth\Desktop\CasADi\casadi-py27-v3.5.5")
from casadi import *
...
class Controller2D(object):
	...
	def operator(self, vars, coeffs, n_constraints, N, dt, ref_v, Lf):
		...
		for i in range(1,N):
			...
			g[x_start + i] = x1 - (x0 + v0 * np.cos(psi0) * dt)
			g[y_start + i] = y1 - (y0 + v0 * np.sin(psi0) * dt)
			g[psi_start + i] = psi1 - (psi0 + (v0/Lf) * delta0 * dt)
			g[v_start + i] = v1 - (v0 + a0 * dt)
			g[cte_start + i] = cte1 - ((f0 - y0) + (v0 * np.sin(epsi0) * dt))
